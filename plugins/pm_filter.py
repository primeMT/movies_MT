 ...
 #   if user_id in ADMINS: return
    ...
    if await db.get_pm_search_status(bot_id):
        ...
        await message.reply_text("<b><i>ɪ ᴀᴍ ɴᴏᴛ ᴡᴏʀᴋɪɴɢ ʜᴇʀᴇ. ꜱᴇᴀʀᴄʜ ᴍᴏᴠɪᴇꜱ ɪɴ ᴏᴜʀ ᴍᴏᴠɪᴇ ꜱᴇᴀʀᴄʜ ɢʀᴏᴜᴘ.</i></b>",
                                 reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("📝 ᴍᴏᴠɪᴇ ꜱᴇᴀʀᴄʜ ɢʀᴏᴜᴘ ", url=f'https://t.me/+zkVCGsbgSH42ZDQ1')]]))
...
async def cb_handler(client: Client, query: CallbackQuery):
    if query.data == "close_data":
        ...
        buttons = [[
            InlineKeyboardButton('☆📸 𝙎𝙚𝙣𝙙 𝙨𝙘𝙧𝙚𝙚𝙣𝙨𝙝𝙤𝙩 📸☆', url=f'https://t.me/movie4sure')
        ],[
      ...
      await client.edit_message_media(
       ...
       await client.edit_message_media(
        ...
        await query.message.edit_text(
			...
			InlineKeyboardButton('🤖 ᴍᴀᴋᴇ ʏᴏᴜʀ ᴏᴡɴ ʙᴏᴛ 🤖', callback_data='Source')
                ],[
                        InlineKeyboardButton('👨‍💻 𝐌𝐲 𝐃𝐞𝐯𝐞𝐥𝐨𝐩𝐞𝐫 👨‍💻',callback_data='mydevelopers')
		],[
        ...
        await query.answer("Meet the mind behind this bot:\n\n👨‍💻 @JohnWickPetBot", show_alert=True)
 
    elif query.data == "Source":
        buttons = [[
            InlineKeyboardButton('Repo', url='https://t.me/johnwickpetbot')
        ],[
       ...
       buttons = [[
	      InlineKeyboardButton('⇆ ᴀᴅᴅ ᴍᴇ ᴛᴏ ʏᴏᴜʀ ɢʀᴏᴜᴘs ⇆', url=f'http://t.me/{temp.U_NAME}?startgroup=start')
              ...
              InlineKeyboardButton('sᴜᴘᴘᴏʀᴛ', url=f'https://t.me/+JwA_RtLZDHs2NTg1'),
       ]]
       reply_markup = InlineKeyboardMarkup(buttons)
       await client.edit_message_media(
          chat_id=query.message.chat.id,
...
async def auto_filter(client, msg, spoll=False , pm_mode = False):
    if not spoll:
        ...
        if not files:
            if settings["spell_check"]:
                ai_sts = await msg.reply_text(f'⏳ ᴀɪ ɪs ᴄʜᴇᴄᴋɪɴɢ ʏᴏᴜʀ sᴘᴇʟʟɪɴɢ...')
    ...
    del_msg = f"\n\n<b>⚠️ ᴛʜɪs ᴍᴇssᴀɢᴇ ᴡɪʟʟ ʙᴇ ᴀᴜᴛᴏ ᴅᴇʟᴇᴛᴇ ᴀꜰᴛᴇʀ <code>{get_readable_time(DELETE_TIME)}</code> ᴛᴏ ᴀᴠᴏɪᴅ ᴄᴏᴘʏʀɪɢʜᴛ ɪssᴜᴇs, ᴘʟᴇᴀsᴇ ғᴏʀᴡᴀʀᴅ ɪᴛ ɪɴ sᴀᴠᴇᴅ ᴍᴀssᴇɢᴇs \n- @LoveMoviesHub</b>" if settings["auto_delete"] else ''
    links = ""
    if settings["link"]:
        btn = []
    ...
    js_ads = f"\n━━━━━━━━━━━━━━━━━━\n <b>{ads_text}</b> \n━━━━━━━━━━━━━━━━━━" if ads_text else ""
  #  del_msg = f"\n\n<b>⚠️ ᴛʜɪs ᴍᴇssᴀɢᴇ ᴡɪʟʟ ʙᴇ ᴀᴜᴛᴏ ᴅᴇʟᴇᴛᴇ ᴀꜰᴛᴇʀ <code>{get_readable_time(DELETE_TIME)}</code> ᴛᴏ ᴀᴠᴏɪᴅ ᴄᴏᴘʏʀɪɢʜᴛ ɪssᴜᴇs \n 💌🇯‌🇴‌🇮‌🇳‌ 🇧‌🇦‌🇨‌🇰‌🇺‌🇵‌ 🇨‌🇭‌🇦‌🇳‌🇳‌🇪‌🇱‌: @LoveMoviesHub</b>" if settings["auto_delete"] else ''
    CAP[key] = cap
